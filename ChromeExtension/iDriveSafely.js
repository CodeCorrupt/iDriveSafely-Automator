...
snd.play();
for (var i=0; i<10; i++) {
  //TODO: For some reason this does not trim off the leading characters when it is run programatically.
  //      This same script when copied and ran in the command line works fine. who knows.
...
var answersPagePattern = "processTestSequenceQuestions";
var answersPageScript = `
...
var testAuthPattern = "displayTestAuth";
var testAuthScript = `
setTimeout(function(){formSubmit()}, 1000);
`;

var finalTestPattern = "displayFinalTestQuestions";
var finalTestScript = `
// Note that the final exam has blank lines
for (var i=0; i<40; i++) {
  var q = document.getElementsByTagName("table")[7].getElementsByTagName("tbody")[0].getElementsByTagName("tr")[(i*6)+2].innerText.replace(/^\s*\d*\s*/,"").trim();
  console.log(q);
  for(var j=1; j<=4; j++) {
    var a = document.getElementsByTagName("table")[7].getElementsByTagName("tbody")[0].getElementsByTagName("tr")[(i*6)+2+j].innerText.trim();
    console.log("    " + a);
  }
}
`;

var finalScorePattern = "displayFinalScore";
var finalScoreScript = `
`;
...
} else if (document.URL.includes(testAuthPattern)) {
  console.log("*** This is a authentication page");
  script = testAuthScript;
} else if (document.URL.includes(finalTestPattern)) {
  console.log("*** This is a final test page");
  script = finalTestScript;
} else {
